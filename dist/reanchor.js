...
const toOutput = (text, candidate, anchor_confidence, reason_code, reanchored, score) => {
    if (!candidate) {
        return {
            start: null,
            end: null,
...
const exactMatches = (text, quote, from = 0, to = text.length) => {
    const matches = [];
...
export const reanchor = (documentText, anchor, params = {}) => {
    ...
    if (allExact.length > 1) {
        const scored = allExact.map((candidate) => {
            ...
            const lev = similarity(anchor.fallback.prefix + anchor.fallback.quote + anchor.fallback.suffix, candPrefix + quote + candSuffix);
