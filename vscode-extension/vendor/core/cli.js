...
const readFileUtf8 = (path) => readFileSync(path, 'utf8');
const revForBytes = (bytes) => `sha256:${createHash('sha256').update(bytes).digest('hex')}`;
const revForSidecarFile = (sidecarPath) => revForBytes(readFileUtf8(sidecarPath));
const resolvePaths = (flags) => {
...
const readAuthor = (flags) => {
    ...
    const verified = verifiedRaw === undefined || verifiedRaw === 'null' ? null : ['true', '1', 'yes', 'on'].includes(verifiedRaw.toLowerCase());
...
const parseIntFlag = (flags, name) => {
    const value = asString(flags.get(name));
    if (!value)
        throw new MdCollabError('BAD_ARGS', `missing --${name}`);
    const parsed = Number.parseInt(value, 10);
    if (!Number.isFinite(parsed))
        throw new MdCollabError('BAD_ARGS', `invalid --${name}`);
    return parsed;
};
const sidecarCounts = (sidecar) => {
    const counts = {
    ...
    for (const thread of sidecar.threads) {
        ...
        if (thread.status === 'resolved')
            counts.resolved += 1;
        if (thread.relevance_state === 'active')
            counts.active += 1;
        if (thread.relevance_state === 'outdated')
            counts.outdated += 1;
        if (thread.relevance_state === 'orphaned')
            counts.orphaned += 1;
    }
    return counts;
};
const maybeEmitAudit = (flags, event, io) => {
    ...
    if (mode === 'file') {
        ...
        if (!path)
            throw new MdCollabError('BAD_ARGS', '--audit=file requires --audit-file <path>');
...
const guardRev = (sidecarPath, flags, revBefore) => {
...
const resolveThreadById = (sidecar, threadId) => {
    const thread = sidecar.threads.find((t) => t.thread_id === threadId);
    if (!thread)
        throw new MdCollabError('THREAD_NOT_FOUND', `thread not found: ${threadId}`);
    return thread;
};
const resolveSuggestionById = (thread, suggestionId) => {
    const suggestion = (thread.suggestions ?? []).find((s) => s.suggestion_id === suggestionId);
    if (!suggestion)
        throw new MdCollabError('SUGGESTION_NOT_FOUND', `suggestion not found: ${suggestionId}`);
    return suggestion;
};
const readSidecarOrThrow = (path) => {
    if (!existsSync(path))
        throw new MdCollabError('BAD_ARGS', `sidecar not found: ${path}`);
...
const extractTextByAnchor = (docText, anchor) => {
...
export const runCli = (argv, io = defaultIo) => {
    ...
    try {
        ...
        if (command === 'comment add') {
            ...
            if (!body)
                throw new MdCollabError('BAD_ARGS', 'missing --body');
        ...
        if (command === 'comment reply') {
            ...
            const body = asString(flags.get('body'));
            if (!threadId || !body)
                throw new MdCollabError('BAD_ARGS', 'missing --thread-id or --body');
            const author = readAuthor(flags);
            return applyMutation({
                command,
                flags,
        ...
        if (command === 'thread resolve' || command === 'thread reopen') {
            ...
            const threadId = asString(flags.get('thread-id'));
            if (!threadId)
                throw new MdCollabError('BAD_ARGS', 'missing --thread-id');
            const author = readAuthor(flags);
            return applyMutation({
                command,
                flags,
                ...
                mutate: (current) => {
                    ...
                    if (command.endsWith('resolve') && existing.status === 'resolved')
                        return current;
                    if (command.endsWith('reopen') && existing.status === 'open')
                        return current;
        ...
        if (command === 'thread refresh-relevance') {
            const { sidecarPath, docPath } = resolvePaths(flags);
            const timelineKind = asString(flags.get('timeline-kind')) ?? 'workspace';
            const context = {
                timelineKind,
        ...
        if (command === 'suggestion propose') {
            ...
            if (!threadId || replacementText === undefined)
                throw new MdCollabError('BAD_ARGS', 'missing --thread-id or --replacement-text');
            const author = readAuthor(flags);
        ...
        if (command === 'suggestion apply') {
            ...
            if (!threadId || !suggestionId)
                throw new MdCollabError('BAD_ARGS', 'missing --thread-id or --suggestion-id');
...
export const main = (argv = process.argv.slice(2), io = defaultIo) => {
