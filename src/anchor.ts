...
import type { Anchor, AnchorBuildOptions, Point, PrimaryAnchor } from './types.js';
...
export const hashQuote = (quote: string): string => sha256(normalizeForHash(quote));

export const hashContext = (prefix: string, suffix: string): string =>
  sha256(normalizeForHash(`${prefix} ${suffix}`));

export const getLineStarts = (text: string): number[] => {
  const starts = [0];
...
export const offsetToPoint = (text: string, offsetUtf16: number): Point => {
  ...
  for (let i = 0; i < starts.length; i++) {
    if (starts[i] <= offsetUtf16) lineIndex = i;
    else break;
  }
...
export const pointToOffset = (text: string, point: Point): number => {
  const starts = getLineStarts(text);
...
export const primaryFromOffsets = (
  text: string,
...
export const validateAnchor = (text: string, anchor: Anchor): void => {
  const start = anchor.primary.start.offset_utf16;
  const end = anchor.primary.end.offset_utf16;
  if (start > end) error('ANCHOR_INVALID', 'start must be <= end');
  if (start < 0 || end > text.length) error('ANCHOR_INVALID', 'offsets out of bounds');
  if (!anchor.fallback.quote) error('ANCHOR_INVALID', 'quote must be non-empty');
